...
$(document).ready(function() {
  ...
  window.traverseTreeData = function(treeData, paths, full_description) {
    if(paths.length == 0) { return []; }

    const currentNode = paths[0];

    let nodeIndex = treeData.findIndex(function(node) { return node.name == currentNode });
    if(nodeIndex == -1) {
      treeData.push({name: currentNode, descriptions: [full_description], children: traverseTreeData([], paths.slice(1))});
    } else {
      // treeData[nodeIndex].descriptions = treeData[nodeIndex].descriptions.push(full_description);
      treeData[nodeIndex].descriptions.push(full_description);
      treeData[nodeIndex].children = traverseTreeData(treeData[nodeIndex].children, paths.slice(1));
    }
  ...
  $('.source_table.highlighted li.covered').live('click', function() {
    ...
    exampleRefs.forEach(function(ref) {
      ...
      const treeNodes = example["file_path"].split('/').slice(1);
      treeNodes.push(example["full_description"]);

      treeData = traverseTreeData(treeData, treeNodes, example["full_description"])

      console.log(treeData);
      // traverseTreeData([], 'specs')


      //exampleHTML += "<div class=\"example_data\"> \
      //  <div class=\"full_description\">" + example["full_description"] + "</div> \
      //  <div class=\"file_path\">" + example["file_path"] + ":" + example["line_number"] + "</div> \
      //</div>";
    })

    const domElement = self.querySelector('.reverse_coverage');

    new TreeView(treeData, domElement);
    //new TreeView([
    ...
    //  }
    //], domElement);
    //$(this).children('.reverse_coverage').html(exampleHTML);
    $(this).children('.reverse_coverage').toggle();
  });
